...
import {V2SwapRouter} from '../modules/takerswap/V2SwapRouter.sol';
import {V3SwapRouter} from '../modules/takerswap/V3SwapRouter.sol';
import {StableSwapRouter} from '../modules/takerswap/StableSwapRouter.sol';
...
import {ITakerNFTMarket} from '../interfaces/ITakerNFTMarket.sol';
...
abstract contract Dispatcher is Payments, V2SwapRouter, V3SwapRouter, StableSwapRouter, Callbacks, LockAndMsgSender {
    ...
    function dispatch(bytes1 commandType, bytes calldata inputs) internal returns (bool success, bytes memory output) {
        ...
        if (command < Commands.FOURTH_IF_BOUNDARY) {
            ...
            if (command == Commands.EXECUTE_SUB_PLAN) {
            ...
            } else if (command == Commands.TAKER_NFT_ETH) {
                ...
                (success, output) = TAKERSWAP_NFT_MARKET.call{value: value}(
                    abi.encodeWithSelector(ITakerNFTMarket.buyTokenUsingETH.selector, collection, tokenId)
                );
                if (!success) output = abi.encodePacked(BuyTakerNFTFailed.selector);
            } else if (command == Commands.TAKER_NFT_WETH) {
                // equivalent: abi.decode(inputs, (address, uint256, uint256))
                address collection;
                uint256 tokenId;
                uint256 price;
                ...
                ITakerNFTMarket(TAKERSWAP_NFT_MARKET).buyTokenUsingWETH(collection, tokenId, price);
            } else {
