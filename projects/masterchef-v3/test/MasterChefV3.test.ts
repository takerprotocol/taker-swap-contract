...
import TakerV3PoolDeployerArtifact from "@pancakeswap/v3-core/artifacts/contracts/TakerV3PoolDeployer.sol/TakerV3PoolDeployer.json";
import TakerV3FactoryArtifact from "@pancakeswap/v3-core/artifacts/contracts/TakerV3Factory.sol/TakerV3Factory.json";
// import TakerV3FactoryOwnerArtifact from "@pancakeswap/v3-core/artifacts/contracts/TakerV3FactoryOwner.sol/TakerV3FactoryOwner.json";
import TakerV3SwapRouterArtifact from "@pancakeswap/v3-periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json";
...
import TakerV3LmPoolDeployerArtifact from "@pancakeswap/v3-lm-pool/artifacts/contracts/TakerV3LmPoolDeployer.sol/TakerV3LmPoolDeployer.json";
...
const nativeCurrencyLabel = "tETH";
...
describe("MasterChefV3", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy factory
    const TakerV3PoolDeployer = await ethers.getContractFactoryFromArtifact(TakerV3PoolDeployerArtifact);
    const takerV3PoolDeployer = await TakerV3PoolDeployer.deploy();

    const TakerV3Factory = await ethers.getContractFactoryFromArtifact(TakerV3FactoryArtifact);
    const takerV3Factory = await TakerV3Factory.deploy(takerV3PoolDeployer.address);

    await takerV3PoolDeployer.setFactoryAddress(takerV3Factory.address);

    const TakerV3SwapRouter = await ethers.getContractFactoryFromArtifact(TakerV3SwapRouterArtifact);
    const takerV3SwapRouter = await TakerV3SwapRouter.deploy(
      takerV3PoolDeployer.address,
      takerV3Factory.address,
      WETH9Address
    );
    ...
    // );
    // const baseTokenUri = "https://nft.takerswap.com/v3/";
    // const nonfungibleTokenPositionDescriptor = await upgrades.deployProxy(NonfungibleTokenPositionDescriptor, [
    //   baseTokenUri,
    // ]);
    // await nonfungibleTokenPositionDescriptor.deployed();
    // TODO:
    await TakerV3SwapRouter.deploy(takerV3PoolDeployer.address, takerV3Factory.address, WETH9Address);
    ...
    // const TakerV3FactoryOwner = await ethers.getContractFactoryFromArtifact(TakerV3FactoryOwnerArtifact);
    // const takerV3FactoryOwner = await TakerV3FactoryOwner.deploy(takerV3Factory.address);
    // await takerV3Factory.setOwner(takerV3FactoryOwner.address);
    ...
    const TakerV3LmPoolDeployer = await ethers.getContractFactoryFromArtifact(TakerV3LmPoolDeployerArtifact);
    const takerV3LmPoolDeployer = await TakerV3LmPoolDeployer.deploy(
      masterChefV3.address
      // takerV3FactoryOwner.address
    );
    // await takerV3FactoryOwner.setLmPoolDeployer(takerV3LmPoolDeployer.address);
    await takerV3Factory.setLmPoolDeployer(takerV3LmPoolDeployer.address);
    await masterChefV3.setLMPoolDeployer(takerV3LmPoolDeployer.address);
    ...
    await tokenA.connect(admin).approve(takerV3SwapRouter.address, ethers.constants.MaxUint256);
    await tokenB.connect(admin).approve(takerV3SwapRouter.address, ethers.constants.MaxUint256);
    await tokenC.connect(admin).approve(takerV3SwapRouter.address, ethers.constants.MaxUint256);
    await tokenD.connect(admin).approve(takerV3SwapRouter.address, ethers.constants.MaxUint256);
    ...
    this.swapRouter = takerV3SwapRouter;
