...
contract MasterChefV3Receiver is Ownable {
    ...
    /// @param _veTaker veTaker token address.
    /// @param _v2Pid The pool id of the dummy pool on the MCV2.
    constructor(IMasterChefV2 _v2, IMasterChefV3 _v3, IERC20 _veTaker, uint256 _v2Pid) {
        MasterChefV2 = _v2;
        MasterChefV3 = _v3;
        veTaker = _veTaker;
        V2_Pid = _v2Pid;

        veTaker.safeApprove(address(_v3), type(uint256).max);
    }
    ...
    /// @notice Harvest pending veTaker tokens from MasterChef V2.
    ...
    /// @notice Withdraw unexpected tokens sent to the receiver, can also withdraw veTaker.
    /// @dev Callable by owner.
