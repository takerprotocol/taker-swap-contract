 ...
 * Receiver will receive veTaker token, then upkeep for MasterChefV3.
 */
contract MasterChefV3KeeperV2 is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    constructor(IMasterChefV3 _V3, IReceiver _receiver, IERC20 _veTaker) {
        MasterChefV3 = _V3;
        Receiver = _receiver;
        veTaker = _veTaker;
    }
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        if (!paused()) {
            ...
            if (veTakerBalanceInReceiver > 0 && latestPeriodEndTime < block.timestamp + bufferSecond) upkeepNeeded = true;
        }
    }
