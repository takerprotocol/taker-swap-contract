...
/**
 * @dev MasterChefV3KeeperV1 was designed to use in ETH chain.
 * Receiver will harvest veTaker from MasterChefV2 pool , then upkeep for MasterChefV3.
 */
contract MasterChefV3KeeperV1 is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    /// @param _veTaker veTaker address.
    /// @param _V2Pid Pid in MasterChefV2.
    constructor(IMasterChefV2 _V2, IMasterChefV3 _V3, IReceiver _receiver, IERC20 _veTaker, uint256 _V2Pid) {
        MasterChefV2 = _V2;
        MasterChefV3 = _V3;
        Receiver = _receiver;
        veTaker = _veTaker;
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        if (!paused()) {
            uint256 penddingVeTakerAmount = MasterChefV2.pendingVeTaker(V2Pid, address(Receiver));
            uint256 veTakerBalanceInReceiver = veTaker.balanceOf(address(Receiver));
            uint256 latestPeriodEndTime = MasterChefV3.latestPeriodEndTime();
            if (penddingVeTakerAmount + veTakerBalanceInReceiver > 0 && latestPeriodEndTime < block.timestamp + bufferSecond)
                upkeepNeeded = true;
        }
    }
