...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of veTaker contract.
    IERC20 public immutable veTaker;
    ...
    uint256 public latestPeriodVeTakerPerSecond;
    ...
    /// @notice Record the veTaker amount belong to MasterChefV3.
    uint256 public veTakerAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, ITakerV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    event NewUpkeepPeriod(
        ...
        uint256 startTime,
        uint256 endTime,
        uint256 veTakerPerSecond,
        uint256 veTakerAmount
    );
    event UpdateUpkeepPeriod(
        uint256 indexed periodNumber,
        uint256 oldEndTime,
        uint256 newEndTime,
        uint256 remainingveTaker
    );
    ...
    /// @param _veTaker The veTaker token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _veTaker, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        veTaker = _veTaker;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
    }

    /// @notice Returns the veTaker per second , period end time.
    /// @param _pid The pool pid.
    /// @return veTakerPerSecond veTaker reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 veTakerPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            veTakerPerSecond = (latestPeriodVeTakerPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the veTaker per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return veTakerPerSecond veTaker reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 veTakerPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            veTakerPerSecond =
                (latestPeriodVeTakerPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending veTaker rewards.
    /// @dev The pending veTaker amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingVeTaker(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function add(uint256 _allocPoint, ITakerV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's veTaker allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotTakerNFT();
    ...
    /// @notice harvest veTaker from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward veTaker reward.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce veTakerAmountBelongToMC.
        if (_token == address(veTaker)) {
            unchecked {
                // In fact balance should always be greater than or equal to veTakerAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= veTakerAmountBelongToMC) {
                    balance -= veTakerAmountBelongToMC;
                } else {
                    // This should never happend.
                    veTakerAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce veTakerAmountBelongToMC.
        if (token == address(veTaker)) {
            unchecked {
                // In fact balance should always be greater than or equal to veTakerAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= veTakerAmountBelongToMC) {
                    balanceToken -= veTakerAmountBelongToMC;
                } else {
                    // This should never happend.
                    veTakerAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of veTaker injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer veTaker token from receiver.
        veTaker.safeTransferFrom(receiver, address(this), _amount);
        // Update veTakerAmountBelongToMC
        unchecked {
            veTakerAmountBelongToMC += _amount;
        }
        ...
        uint256 veTakerPerSecond;
        uint256 veTakerAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingVeTaker = ((latestPeriodEndTime - currentTime) * latestPeriodVeTakerPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingVeTaker);
            veTakerAmount += remainingVeTaker;
        }
        veTakerPerSecond = (veTakerAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodVeTakerPerSecond = veTakerPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, veTakerPerSecond, veTakerAmount);
    }

    /// @notice Update veTaker reward for all the liquidity mining pool.
    ...
    /// @notice Update veTaker reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer veTaker.
    /// @param _to The veTaker receiver address.
    /// @param _amount Transfer veTaker amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = veTaker.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update veTakerAmountBelongToMC
            unchecked {
                if (veTakerAmountBelongToMC >= _amount) {
                    veTakerAmountBelongToMC -= _amount;
                } else {
                    veTakerAmountBelongToMC = balance - _amount;
                }
            }
            veTaker.safeTransfer(_to, _amount);
        }
    }
