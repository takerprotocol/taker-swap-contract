import { abi as ITakerV3PoolABI } from '@pancakeswap/v3-core/artifacts/contracts/interfaces/ITakerV3Pool.sol/ITakerV3Pool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { ITakerV3Pool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain-types'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [ITakerV3Pool, ITakerV3Pool, ITakerV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, ITakerV3PoolABI, wallet)) as [
      ITakerV3Pool,
      ITakerV3Pool,
      ITakerV3Pool
    ]
  ...
  let pools: [ITakerV3Pool, ITakerV3Pool, ITakerV3Pool]
  ...
  describe('#exactInput', () => {
    ...
    it.skip('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestTakerV3Callee')
