...
/// @notice An example contract using the TakerSwap V3 flash function
contract PairFlash is ITakerV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `TakerV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        ITakerV3Pool pool = ITakerV3Pool(PoolAddress.computeAddress(deployer, poolKey));
