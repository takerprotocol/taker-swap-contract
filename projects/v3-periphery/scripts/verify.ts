...
async function main() {
  ...
  const deployedContracts_v3_periphery = await import(`../deployments/${networkName}.json`)

  // Verify swapRouter
  console.log('Verify swapRouter')
  await verifyContract(deployedContracts_v3_periphery.SwapRouter, [
    deployedContracts_v3_core.TakerV3PoolDeployer,
    deployedContracts_v3_core.TakerV3Factory,
    config.WNATIVE,
  ])
  await sleep(10000)
  ...
  // Verify takerInterfaceMulticall
  console.log('Verify takerInterfaceMulticall')
  await verifyContract(deployedContracts_v3_periphery.TakerInterfaceMulticall)
  await sleep(10000)
  ...
  //verify TakerInterfaceMulticallV2
  console.log('Verify TakerInterfaceMulticallV2')
  await verifyContract(deployedContracts_v3_periphery.TakerInterfaceMulticallV2)
  await sleep(10000)
}
