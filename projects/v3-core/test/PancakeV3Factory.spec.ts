...
describe('TakerV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: TakerV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as TakerV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as TakerV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [TakerV3Factory, TakerV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('TakerV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('TakerV3Pool')
