...
import { TestTakerV3SwapPay } from '../typechain-types/contracts/test/TestTakerV3SwapPay'
...
import { TestTakerV3ReentrantCallee } from '../typechain-types/contracts/test/TestTakerV3ReentrantCallee'
...
describe('TakerV3Pool', () => {
  ...
  let factory: TakerV3Factory
  let pool: MockTimeTakerV3Pool

  let swapTarget: TestTakerV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeTakerV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestTakerV3ReentrantCallee')
      ).deploy()) as TestTakerV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestTakerV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestTakerV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestTakerV3SwapPay
