...
import { TakerV3PoolSwapTest } from '../typechain-types/contracts/test/TakerV3PoolSwapTest'
...
describe('TakerV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('TakerV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as TakerV3PoolSwapTest
          ...
          let tester: TakerV3PoolSwapTest
