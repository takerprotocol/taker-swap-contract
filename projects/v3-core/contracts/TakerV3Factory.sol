...
/// @title Canonical TakerSwap V3 factory
/// @notice Deploys TakerSwap V3 pools and manages ownership and control over pool protocol fees
contract TakerV3Factory is ITakerV3Factory {
    /// @inheritdoc ITakerV3Factory
    ...
    function createPool(
        ...
        pool = ITakerV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ITakerV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ITakerV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ITakerV3Pool(pool).setLmPool(lmPool);
    }
}
