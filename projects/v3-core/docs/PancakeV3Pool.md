...
The contract that deployed the pool, which must adhere to the ITakerV3Factory interface
...
```solidity
struct TakerV3Pool.Slot0 slot0
```
...
```solidity
struct TakerV3Pool.ProtocolFees protocolFees
```
...
```solidity
contract ITakerV3LmPool lmPool
```
...
_Prevents calling a function from anyone except the address returned by ITakerV3Factory#owner()_
...
```solidity
function setLmPool(contract ITakerV3LmPool _lmPool) external
```
